...
func InitRouterV1(groupV1 *gin.RouterGroup, config *config.CKManConfig, signal chan os.Signal) {
	...
	configController := controller.NewConfigController(signal, config, WrapMsg)
	...
	groupV1.POST(fmt.Sprintf("/ck/table/migrate/:%s", controller.ClickHouseClusterPath), ckController.MigrateTable)
	...
	groupV1.POST(fmt.Sprintf("/ck/rebalance_info/:%s", controller.ClickHouseClusterPath), ckController.GetRebalanceInfo)
	...
	groupV1.GET(fmt.Sprintf("/metric/query_metric/:%s", controller.ClickHouseClusterPath), metricController.QueryMetric)
	groupV1.GET("/version", configController.GetVersion)
	groupV1.GET("/instances", configController.GetInstances)
