...
func InitRouterV2(groupV2 *gin.RouterGroup, config *config.CKManConfig, signal chan os.Signal) {
	...
	configController := controller.NewConfigController(signal, config, WrapMsg2)
	...
	dataManageController := controller.NewDataManageController(config, WrapMsg2)
	...
	groupV2.GET(fmt.Sprintf("/ck/vm/:%s", controller.ClickHouseClusterPath), ckController.GetMaterializedViewStatus)
	...
	groupV2.GET(fmt.Sprintf("/ck/backgroundpool/:%s", controller.ClickHouseClusterPath), ckController.GetBackgroundPool)
	...
	groupV2.GET(fmt.Sprintf("/zk/replicated-queue/:%s", controller.ClickHouseClusterPath), zkController.GetReplicatedQueue)
	...
	groupV2.GET("/instances", configController.GetInstances)
	...
	groupV2.POST(fmt.Sprintf("/data-manage/backup/:%s", controller.ClickHouseClusterPath), dataManageController.BackupData)
	groupV2.POST(fmt.Sprintf("/data-manage/restore/:%s", controller.ClickHouseClusterPath), dataManageController.RestoreData)
	groupV2.GET(fmt.Sprintf("/data-manage/backup/:%s", controller.ClickHouseClusterPath), dataManageController.GetBackupHistory)
	groupV2.DELETE("/data-manage/backup/:backupId", dataManageController.DeleteBackup)
	groupV2.GET("/data-manage/backup_history/:backupId", dataManageController.GetBackupById)
}
